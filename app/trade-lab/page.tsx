        ...
        {selected.map((asset) => (
          <article className={`trade-asset ${asset.kind === "pick" ? "pick-asset" : ""}`} key={asset.id}>
            ...
            <div className="trade-asset-copy">
              <b>{asset.name}</b>
              {asset.kind === "player"
                ? <small>{asset.position}{asset.nflTeam ? ` · ${asset.nflTeam}` : ""}{asset.starter ? " · Starter" : " · Bench"}{asset.projectedPoints !== null ? ` · ${asset.projectedPoints.toFixed(1)} projected` : ""}</small>
...
export default function TradeLabPage() {
  ...
  const assets = useMemo<Asset[]>(() => {
    const players: PlayerAsset[] = league.teams.flatMap((team) => team.players.map((playerId) => {
      ...
      const model = tradeValueForPlayer(profile, starter, format, { rosterPositions: league.rosterPositions, scoringSettings: league.scoringSettings }, snapshotMap.get(playerId));
      return {
        ...
        starter, value: model.value, confidence: model.confidence, projectedPoints: model.projectedPoints,
        recentActualPoints: model.recentActualPoints, productionAdjustment: model.productionAdjustment
      };
    }));
  ...
  }, [format, league.draftPicks, league.rosterPositions, league.scoringSettings, league.season, league.teams, profiles, snapshotMap]);
  ...
  return (
    <AppShell>
      <div className="page-heading trade-page-heading"><div><span className="eyebrow">Format-aware trade evaluation</span><h1>Trade Lab</h1><p>League-specific Sleeper projections, recent performance, roster fit, and—when available—owned dynasty draft picks.</p></div><span className="pill">{source === "demo" ? "Sample rosters" : `${source} rosters`}</span></div>
      ...
      {loadingPlayers && <div className="connection-message">Loading Sleeper projections, actual stats, and league-specific values…</div>}
      {playerError && <div className="connection-message error">{playerError}</div>}
      {intelligenceState.data && (
        <div className="trade-intelligence-strip">
          <span><b>Week {intelligenceState.data.projectionWeek}</b> Sleeper projections</span>
          <span><b>{intelligenceState.data.accuracy.meanAbsoluteError === null ? "N/A" : intelligenceState.data.accuracy.meanAbsoluteError.toFixed(2)}</b> projection MAE</span>
          <span><b>{intelligenceState.data.storedWeeks.length}</b> weeks stored</span>
        </div>
      )}
      {intelligenceState.data?.storageStatus === "migration-required" && <div className="connection-message error">Run the included Supabase 1.3C migration to preserve weekly player snapshots.</div>}
      ...
      <div className="trade-grid trade-grid-clean">
        ...
        <div className={`trade-verdict ${isComplete && Math.abs(difference) > tolerance ? "has-advantage" : ""}`}>
          ...
          <div className="verdict-breakdown"><span>Players {playerValueA}–{playerValueB}</span><span>Weekly projection {projectedA.toFixed(1)}–{projectedB.toFixed(1)}</span>{format === "dynasty" && <span>Picks {pickValueA}–{pickValueB}</span>}<span>Roster fit {fitDeltaA >= 0 ? "+" : ""}{fitDeltaA} / {fitDeltaB >= 0 ? "+" : ""}{fitDeltaB}</span></div>
        </div>
      ...
      <section className="panel trade-method-panel">
        ...
        <div className="trade-method-grid">
          <article><strong>Players</strong><p>Sleeper weekly projection, recent actual performance, position, starter role, health, age, and league format.</p></article>
        ...
        <p className="model-disclaimer">Sleeper statistics are rescored using this league&apos;s settings. Values cannot be edited. Future picks without a known slot are projections, not guarantees. FantasyNextMove evaluates the trade but never sends an offer to your league.</p>
      </section>
    </AppShell>
  );
}
